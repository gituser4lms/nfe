...
abstract class quiz_attempts_report_table extends table_sql {
    ...
    public function col_fathername($attempt) {
            global $DB;
	    $userid = $attempt->userid;
	    $type_father_name = $DB->get_field_sql("SELECT f.id 
              FROM {user_info_field} AS f
              WHERE f.shortname = 'father_hide'");
	    $fathername = $DB->get_field_sql("SELECT data FROM {user_info_data} WHERE fieldid = '$type_father_name' AND userid='$userid'");
            return  $fathername;
    }
    ...
    public function col_school($attempt) {
            global $DB;
	    $userid = $attempt->userid;
	    $type_school = $DB->get_field_sql("SELECT f.id 
              FROM {user_info_field} AS f
    ...
    public function col_township($attempt) {
            global $DB;
	    $userid = $attempt->userid;
	    $type_township = $DB->get_field_sql("SELECT f.id 
              FROM {user_info_field} AS f
    ...
    public function col_hometownship($attempt) {
            global $DB;
	    $userid = $attempt->userid;
	    $type_hometownship = $DB->get_field_sql("SELECT f.id 
              FROM {user_info_field} AS f
